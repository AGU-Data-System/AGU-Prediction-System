...
def day_type(df, path):
    ...
    """
    day_type_result = [0] * len(df)
    holly_days = load_file(path)
    for k, idx in enumerate(df.index):
        date_str = df.loc[idx, DataFrameColumns.DATE.value].strftime("%Y-%m-%d")
        date = datetime.strptime(date_str, "%Y-%m-%d")
        for j in holly_days:
            holly_day = datetime.strptime(j, "%Y-%m-%d")
            if date.month == holly_day.month and date.day == holly_day.day:
                day_type_result[k] = 1
                break
        if date.weekday() in [5, 6]:
            day_type_result[k] = 1
    return day_type_result
